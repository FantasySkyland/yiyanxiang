package com.example.administrator.italker.ui.socketclient;
...
import com.example.administrator.italker.ui.socketclient.helper.SocketClientReceivingDelegate;
import com.example.administrator.italker.ui.socketclient.helper.SocketClientSendingDelegate;
...
import com.example.administrator.italker.ui.socketclient.helper.SocketInputReader;
import com.example.administrator.italker.ui.socketclient.helper.SocketPacket;
...
public class SocketClient {
    ...
    public Socket getRunningSocket() {
        if (this.runningSocket == null) {
            ...
            try{
            	this.runningSocket.setKeepAlive(true);
    ...
    public SocketClient setLastSendHeartBeatMessageTime(long lastSendHeartBeatMessageTime) {
    ...
    public SocketClient setLastReceiveMessageTime(long lastReceiveMessageTime) {
    ...
    private void __i__onTimeTick() {
       ...
       //Log.i("daley",String.format("asdas %b %d",getSocketConfigure().getSocketPacketHelper().isSendTimeoutEnabled(),getLastSendMessageTime()) );
        if (getSocketConfigure().getSocketPacketHelper().isSendTimeoutEnabled()
                && getLastSendMessageTime() != NoSendingTime) {
            //Log.i("daley",String.format("asdas %d",currentTime - getLastSendMessageTime()) );
            ...
            try {
                while (self.isConnected()
                    ...
                    try {
                        ...
                        self.__i__onSendPacketEnd(packet);
                        self.setSendingPacket(null);
                        //Log.i("daley",String.format("setnotime"));
                        self.setLastSendMessageTime(NoSendingTime);
                    }
                    catch (IOException e) {
                        e.printStackTrace();
        ...
        public void run() {
            ...
            try {
                while (self.isConnected()
                    ...
                    //Log.i("daley",String.format("read recive：%d %d",headerDataLength,trailerDataLength));
                    ...
                    if (self.getSocketConfigure().getSocketPacketHelper().getReadStrategy() == SocketPacketHelper.ReadStrategy.AutoReadByLength) {
                        ...
                        //Log.i("daley","AutoReadToTrailer");
                        if (trailerDataLength > 0) {
                            byte[] data = self.getSocketInputReader().readToData(trailerData, false);
                            //Log.i(Consts.LOG_TAG,String.format("设置接收时间:%d",System.currentTimeMillis()));
                            self.setLastReceiveMessageTime(System.currentTimeMillis());
                            packet.setData(data);
                            packet.setTrailerData(trailerData);
                            if(data!=null){
                            ...
                            //Log.i("daley",String.format("receivedPacketLength：%d",receivedPacketLength));
                        }
                        else {
                            self.__i__onReceivePacketCancel(packet);
                            self.setReceivingResponsePacket(null);
                            //Log.i("daley",String.format("receivedPacketLength2：%d",receivedPacketLength));
                        }
                    }
